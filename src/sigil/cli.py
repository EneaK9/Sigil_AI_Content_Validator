"""Unified ``sigil`` command-line interface.

Usage:
    sigil validate [--platform PLATFORM] [--campaign-id UUID] [--limit N]
    sigil report   [--platform PLATFORM] [--severity SEVERITY] [--limit N]
    sigil stats
    sigil scrape   [--platform PLATFORM] [--limit N]
"""
...
from sigil.logging import configure_logging
from sigil.pipeline.pipeline import (
    ValidationStatus,
    fetch_violations,
    get_validation_stats,
    validate_scraped_posts,
)
from sigil.scraper.bulk_scrape import run_bulk_scrape
...
async def cmd_scrape(args: argparse.Namespace) -> int:
    """Run a one-off bulk scrape of the configured campaigns (manual/ops)."""
    results = await run_bulk_scrape(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="sigil",
        description="Scrape, validate, and report on social posts against platform policy",
    )
    ...
    scrape_parser = subparsers.add_parser(
        "scrape",
        help="Run a one-off bulk scrape of configured campaigns (manual/ops)",
    )
