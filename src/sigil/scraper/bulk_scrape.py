"""One-off bulk scrape, used immediately without the scheduler.

Exposed to operators via ``sigil scrape``. It bypasses the scheduler's budget
and concurrency guards and runs the configured campaigns to completion in
sequence, so it is a manual/ops tool rather than the steady-state path.
"""
...
from sigil.scraper.apify.client import ApifyRunStatus, ApifyService
...
from sigil.scraper.models import Campaign, RunStatus
from sigil.scraper.platforms.base import get_scraper

import sigil.scraper.platforms  # noqa: F401 - register adapters
